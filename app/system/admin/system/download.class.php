...
class download extends admin {
	...
	public function doc() {
		...
		if ($suc == 'suc') {
			return $this->suc_data('check', '1', "{$_M['word']['file_permissions']}....", 0, 'confirm', $this->confirm("系统升级文档：", $html, $_M['word']['updatelater'], $_M['word']['updatenow']));
		}else{
		...
		if ($this->checkfilepower($updatefile)) {
			...
			if ($suc == 'suc') {
				...
				if ($re == 1) {
					...
					if($nopower){
						return $this->fail_data($_M['word']['following_documents'], $_M['word']['following_documents'].$nopower);
					}else{
