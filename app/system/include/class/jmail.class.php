...
class jmail{
	...
	public function set_sys_mailbox() {
		global $_M;		
		$this->set_send_mailbox($_M['config']['met_fd_usename'], $_M['config']['met_fd_fromname'], $_M['config']['met_fd_usename'], $_M['config']['met_fd_password'], $_M['config']['met_fd_smtp']);
	}
	
	/** 
	 * 设置发件邮箱
	 */	
	public function set_send_mailbox($fromuser, $fromname, $usename, $usepassword, $smtp, $port, $way) {
		global $_M;
