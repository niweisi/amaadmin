define(function(require, exports, module) {
	...
	exports.execution = function(){
		...
		function table_check(){
			var check = $(".ui-table td input[type='checkbox']");
			if(check.length>0){
				var v = check.eq(0).parents(".ui-table").find("input[data-table-chckall]").eq(0).attr("data-table-chckall");
				$(document).on('change',".ui-table td input[type='checkbox']",function(){
		...
		$(document).on( 'init.dt', function ( e, settings ) {
			...
			common.modifytick();/*修改内容后自动勾选对应选项*/
